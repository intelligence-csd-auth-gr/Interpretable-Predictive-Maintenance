...
class interactive_iml_tool:
    def __init__(self, predictor, iml_methods, nn_forecasters, dataset, sensors, target_scaler):
    ...
    def load_instance(self, instance):
        ...
        [timestep_weights, sensor_weights], _ = self.ipca.find_importance(self.instance,300,model)
        timestep_weights = timestep_weights.reshape(700)
        pca_weights = [timestep_weights, sensor_weights]
        
        self.weights_dict = {'LioNets':[lionet_weights],'Interpretable PCA':pca_weights}
    ...
    def modify(self, weights, sens, mod, uni_mod_val=0, uni_wght_sign=1, select_rul=0, xyz7_tm=0, forecast_option=6, mod_range=(1,50)):
        ...
        if mod == 1: # Uniform
            ...
            mod_instance[start-1:end, sens] = 0    
    ...
    def moded_instance_statistics(self, temp_instance, iml_method):
        ...
        if iml_method == 'Interpretable PCA':
            ...
            [timestep_weights, sensor_weights], local_prediction  = self.ipca.find_importance(temp_instance,300,model)
            weights = timestep_weights.reshape(700)
        else:
        ...
        if iml_method == 'Interpretable PCA': 
            ...
            sens_stats = [sensors_mean,sensors_std,sensors_min,sensors_max]

        return [real_prediction, local_prediction], sensors_all, sens_stats
    
    def recommend_modifications(self, iml_method):
        
        if iml_method == 'Interpretable PCA':
            sens_importance = self.weights_dict[iml_method][1]
        else:
            _, _, original_sens_stats =  self.original_instance_statistics[iml_method]
            sens_importance  =  original_sens_stats[0] #The mean of weights per sensor
        indexed = list(enumerate(sens_importance))
